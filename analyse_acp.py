...
def charger_data(file, sheet):
    try:
        ...
        # Correction pour les dates : On cherche la ligne qui contient "/"
        idx_header = 1
        ...
        for i, col in enumerate(headers):
            c_str = str(col).strip()
            if c_str == "nan" or c_str == "": c_str = f"Info_{i}"
            if c_str in counts:
                counts[c_str] += 1
        ...
        # Traitement spécifique demandé pour ProductionPlanning
        if sheet == "ProductionPlanning":
            # Supprimer colonnes A et B (indices 0 et 1)
            df_data = df_data.iloc[:, 2:]
            # Garder "atterissage ACP 29" sur la première ligne
            if not df_data.empty:
                # On force la valeur dans la colonne Info_2 (ajustée après suppression)
                col_target = [c for c in df_data.columns if "Info" in c][1]
                df_data.iloc[0, df_data.columns.get_loc(col_target)] = "atterissage ACP 29"

        # Remplissage des cellules fusionnées
...
if source:
    ...
    if not df_brut.empty:
        df = df_brut.copy()
        # Identification des colonnes de dates (JJ/MM/AA ou format Excel date)
        ...
        with tab_graph:
            st.info("Sélectionnez des filtres pour analyser les données graphiquement.")
